...
public class AccionesEstrategicasDAOImpl implements AccionesEstrategicasDAO {
    ...
    public BeanPlaneamiento getAccionesEstrategicas(BeanPlaneamiento objBeanObjetivos, String usuario) {
        sql = "SELECT VAEI_DESCRIPCION, VAEI_ABREVIATURA, NAEI_PRIORIDAD, DAEI_FECHA_INICIO, "
                + "DAEI_FECHA_FINAL, CUNIDAD_MEDIDA_CODIGO, CESTADO_CODIGO "
                + "FROM OPREFA_PEI_ACCIONES_ESTRATEGIC WHERE "
                + "CPERIODO_CODIGO=? AND "
                + "NOEI_CODIGO=? AND "
                + "NAEI_CODIGO=?";
    ...
    public int iduAccionesEstrategicas(BeanPlaneamiento objBeanObjetivos, String usuario) {
        ...
        try (CallableStatement cs = objConnection.prepareCall(sql)) {
            ...
            cs.setDate(7, objBeanObjetivos.getFechaInicio());
            cs.setDate(8, objBeanObjetivos.getFechaFinal());
            cs.setString(9, objBeanObjetivos.getUnidadMedida());
            cs.setString(10, objBeanObjetivos.getEstado());
            cs.setString(11, usuario);
            cs.setString(12, objBeanObjetivos.getMode());
            s = cs.executeUpdate();
            cs.close();
        } catch (SQLException e) {
            ...
            objBnMsgerr.setTabla("OPREFA_PEI_ACCIONES_ESTRATEGIC");
