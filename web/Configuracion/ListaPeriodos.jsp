<%-- 
    ...
    Created on : 22/03/2020, 07:49:20 PM
    Author     : H-URBINA-M
--%>
...
<script type="text/javascript">
    ...
    var msg = "";
    var lista = new Array();
    <c:forEach var="d" items="${objPeriodos}">
    var result = {codigo: '${d.codigo}', descripcion: '${d.descripcion}', igv: '${d.importe}', estado: "${d.estado}"};
    lista.push(result);
    </c:forEach>
    $(document).ready(function () {
        ...
        $("#div_GrillaPrincipal").jqxGrid({
            ...
            rendertoolbar: function (toolbar) {
                ...
                ButtonExportar.click(function (event) {
                    $("#div_GrillaPrincipal").jqxGrid('exportdata', 'xls', 'Listado de Periodos');
                });
            ...
            columns: [
                {text: 'PERIODO', dataField: 'codigo', width: '10%', align: 'center', cellsAlign: 'center', cellclassname: cellclass},
                {text: 'NOMBRE', dataField: 'descripcion', width: '65%', align: 'center', cellsAlign: 'left', cellclassname: cellclass},
                {text: 'IGV', dataField: 'igv', width: '10%', align: 'center', cellsAlign: 'right', cellsFormat: 'f2', cellclassname: cellclass},
            ...
            function _createElements() {
                ...
                $('#div_VentanaPrincipal').jqxWindow({
                    ...
                    initContent: function () {
                        $("#txt_Periodo").jqxInput({placeHolder: "PERIODO", width: 200, height: 20, minLength: 4, maxLength: 4});
                        $("#txt_Descripcion").jqxInput({placeHolder: "NOMBRE", width: 490, height: 20, minLength: 2, maxLength: 100});
                        $("#div_IGV").jqxNumberInput({width: 120, height: 20, max: 999, digits: 3, decimalDigits: 2});
                        $('#btn_Cancelar').jqxButton({width: '65px', height: 25});
                        $('#btn_Guardar').jqxButton({width: '65px', height: 25});
                        $('#btn_Guardar').on('click', function () {
                            $('#frm_Periodo').jqxValidator('validate');
                        });
                        $('#frm_Periodo').jqxValidator({
                            rules: [
                                {input: '#txt_Descripcion', message: 'Ingrese el Nombre del Periodo!', action: 'keyup, blur', rule: 'required'},
                            ]
                        });
                        $('#frm_Periodo').jqxValidator({
                            onSuccess: function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (dato.length === 2) {
                        ...
                        $("#div_IGV").val(dato[1]);
                        $('#txt_Periodo').jqxInput({disabled: true});
                    }
                }
            });
        ...
        function fn_GrabarDatos() {
            var codigo = $("#txt_Periodo").val();
            var descripcion = $("#txt_Descripcion").val();
            var igv = $("#div_IGV").val();
            $.ajax({
                type: "POST",
                url: "../IduPeriodos",
                data: {mode: mode, codigo: codigo, descripcion: descripcion, igv: igv},
                success: function (data) {
                    msg = data;
                    if (msg === "GUARDO") {
                        $.confirm({
...
<div id="div_VentanaPrincipal" style="display: none">
    <div>
        <span style="float: left">DATOS DEL PERIODO</span>
    </div>
    <div style="overflow: hidden">
        <form id="frm_Periodo" name="frm_Periodo" method="post" >
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <td class="inputlabel">Periodo : </td>
                    <td><input type="text" id="txt_Periodo" name="txt_Periodo" style="text-transform: uppercase;"/></td>
                </tr>
                <tr>
